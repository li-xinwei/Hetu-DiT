...
from hetu_dit.envs import CUDA_VERSION, TORCH_VERSION, PACKAGES_CHECKER
...
class SequenceParallelConfig:
    ...
    def __post_init__(self):
        ...
        if has_gpu() and not HAS_LONG_CTX_ATTN and self.sp_degree > 1:
            raise ImportError(
        ...
        if has_gpu() and not HAS_FLASH_ATTN and self.ring_degree > 1:
            raise ValueError(
                "Flash attention not found. Ring attention not available. Please set ring_degree to 1"
            )
...
class PipeFusionParallelConfig:
    ...
    def __post_init__(self):
        ...
        if self.num_pipeline_patch is None:
            ...
            logger.info(
                f"Pipeline patch number not set, using default value {self.pp_degree}"
            )
        ...
        if self.pp_degree == 1 and self.num_pipeline_patch > 1:
            logger.warning(
                "Pipefusion degree is 1, pipeline will not be used,"
                "num_pipeline_patch will be ignored"
            )
...
class ParallelConfig:
    ...
    # Text encoder parallel
    text_encoder_tp_config: Optional[TensorParallelConfig] = None
    is_serving: bool = False
    ...
    def __post_init__(self):
        ...
        # Text encoder parallel
        if self.text_encoder_tp_config is None:
            self.text_encoder_tp_degree = 1
        else:
            self.text_encoder_tp_degree = self.text_encoder_tp_config.tp_degree
