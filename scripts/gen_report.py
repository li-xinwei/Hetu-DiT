...
def render_report(
    ...
    *,
    ...
    parts.append("## Calibration — two-source validation")
    parts.append("")
    parts.append("Sim constants come from the 2026-05-06 RunPod H100 SXM 4-GPU session. Those are end-to-end Hetu-DiT timings (Ray + K3s + GPU model load), so a single session yields one observation per constant. To check that the values are not numerology, we ran an **independent microbenchmark** on a separate hardware tier (community RTX 3090 24 GB, \\$0.22/hr) on 2026-05-11 — the raw JSON is in `data/microbench_rtx3090_2026-05-11.json`.")
    parts.append("")
    parts.append("### Source A — Hetu-DiT cstrace (H100 SXM 4-GPU, 2026-05-06)")
    parts.append("")
    ...
    parts.append("")
    parts.append("### Source B — independent RTX 3090 microbenchmark (2026-05-11)")
    parts.append("")
    parts.append("Run on a fresh community pod, isolating the two physical components the sim collapses into a single number: (i) **H2D bandwidth** at 1 / 5 / 17 GB tensor sizes; (ii) **inference jitter** across 5 SD-1.5 1024² runs after warmup.")
    parts.append("")
    parts.append("| Microbenchmark (5 reps each, RTX 3090 24 GB) | Mean | Stdev | Relative stdev |")
    parts.append("| --- | ---: | ---: | ---: |")
    parts.append("| H2D copy, 1 GB pinned tensor | 64.5 ms | 1.2 ms | 1.9 % |")
    parts.append("| H2D copy, 5 GB pinned tensor | 323.8 ms | 0.15 ms | 0.05 % |")
    parts.append("| H2D copy, 17 GB pinned tensor | **727 ms** | 0.3 ms | 0.04 % |")
    parts.append("| SD-1.5 inference, 1024² 20 steps | **4.394 s** | 0.013 s | **0.29 %** |")
    parts.append("| pipeline.to('cuda'), SD-1.5 (~5 GB FP16) | 0.206 s | — | — |")
    parts.append("")
    parts.append("### What B tells us about A")
    parts.append("")
    parts.append("- **`T_BIND_COLD_L2` = 1.0 s is consistent.** The pure H2D copy of 17 GB on RTX 3090 PCIe Gen3 takes 727 ms. On H100 SXM PCIe Gen5 (≈4× the H2D bandwidth) the same copy would take ~270 ms. The 2026-05-06 measurement of 1.04 s on H100 SXM therefore has ~770 ms of **non-H2D overhead** — this matches the `cudaMemcpyAsync` per-tensor launch overhead exposed by the BPS post-mortem (commit `cebd01d`): PyTorch's `.to()` issues one launch per parameter tensor, and SD3 has ~500–1000 of them.")
    parts.append("- **`T_INFER` jitter assumption holds.** Sim treats inference as deterministic. Microbench gives a relative stdev of **0.29 %** across 5 reps (4.382, 4.404, 4.410, 4.391, 4.382 s) — empirically zero compared to the seed-level workload jitter the report's CIs already capture.")
    parts.append("- **Hardware delta is acknowledged, not hidden.** The microbench runs SD-1.5 5 GB FP16 on RTX 3090 PCIe Gen3 (16.6 GB/s steady, 25.1 GB/s peak); the sim represents SD3 17 GB FP16 on H100 SXM PCIe Gen5 (~64 GB/s). Comparison is not apples-to-apples on absolute latency, but on (a) the bandwidth law (linear in bytes), (b) the per-tensor launch overhead ceiling, and (c) inference jitter — all three of which transfer.")
    parts.append("")
